package com.ztc.testcenter.question.gre.generator;

import com.ztc.testcenter.file.service.FileService;
import com.ztc.testcenter.question.gre.repository.QuestionTemplateRepository;
...
public class QuestionsGenerator {
    ...
    @Autowired
    public QuestionsGenerator(QuestionService questionService, QuestionTemplateRepository questionTemplateRepository, FileService fileService) {
    ...
    private List<QuantitativeComparisonQuestion> getQuantitativeComparisonQuestionSamples() {
        ...
        quantitativeComparisonQuestion.setImage(fileService.get((long) 1));
    ...
    private List<QuantitativeSingleAnswerQuestion> getQuantitativeSingleAnswerQuestionSamples() {
        ...
        quantitativeSingleAnswerQuestion.setImage(fileService.get((long) 2));
        ...
        quantitativeSingleAnswerQuestion.setImage(fileService.get((long) 3));
    ...
    private List<DataInterpretationSetQuestion> getDataInterpretationSetQuestionSamples() {
        ...
        dataInterpretationSetQuestion.setImage(fileService.get((long) 4));
        ...
        dataInterpretationSetQuestion.setText("");
        dataInterpretationSetQuestion.setImage(fileService.get((long) 5));
    ...
    private List<WritingQuestion> getWritingQuestionSamples() {
        ...
        writingQuestion.setText("<p>As people rely more and more on technology to solve problems, the ability of humans to think for themselves will surely deteriorate.</p>");
        writingQuestion.setDescription("<p>Write a response in which you discuss the extent to which you agree or disagree with the statement and explain your reasoning for the position you take. In developing and supporting your position, you should consider ways in which the statement might or might not hold true and explain how these considerations shape your position.</p>");
        ...
        writingQuestion.setText("<p>A nation should require all of its students to study the same national curriculum until they enter college.</p>");
        writingQuestion.setDescription("<p>Write a response in which you discuss the extent to which you agree or disagree with the recommendation and explain your reasoning for the position you take. In developing and supporting your position, describe specific circumstances in which adopting the recommendation would or would not be advantageous and explain how these examples shape your position.</p>");
        writingQuestion.setType(WritingQuestion.Type.ANALYZE_AND_ISSUE);
        ...
        writingQuestion.setDescription("<p>Write a response in which you examine the stated and/or unstated assumptions of the argument. Be sure to explain how the argument depends on the assumptions and what the implications are if the assumptions prove unwarranted.</p>");
        writingQuestion.setType(WritingQuestion.Type.ANALYZE_AND_ARGUMENT);

        writingQuestionSamples.add(writingQuestion);

        writingQuestion = new WritingQuestion();
        writingQuestion.setText("<p>The following is a memorandum from the business manager of a television station. </p><p><br></p><p>\"Over the past year, our late-night news program has devoted increased time to national news and less time to weather and local news. During this time period, most of the complaints received from viewers were concerned with our station's coverage of weather and local news. In addition, local businesses that used to advertise during our late-night news program have just canceled their advertising contracts with us. Therefore, in shop to attract more viewers to the program and to avoid losing any further advertising revenues, we should restore the time devoted to weather and local news to its former level.\"</p>");
        writingQuestion.setDescription("<p>Write a response in which you discuss what specific evidence is needed to evaluate the argument and explain how the evidence would weaken or strengthen the argument.</p>");
        writingQuestion.setType(WritingQuestion.Type.ANALYZE_AND_ARGUMENT);
    ...
    private void createNumericQuestion(Integer number, AbstractNumericQuestion question) {
        question.setText("This is a sample Numeric gre " + number);
        question.setDifficulty(Difficulty.values()[random.nextInt(3)]);
        question.setDifficultyLevel(DifficultyLevel.values()[random.nextInt(5)]);
    ...
    private void createQuantitativeMultipleAnswerQuestion(Integer number, AbstractQuantitativeMultipleAnswerQuestion question) {
        question.setText("This is a sample Quantitative Multiple Answer gre " + number);
        question.setDifficulty(Difficulty.values()[random.nextInt(3)]);
        question.setDifficultyLevel(DifficultyLevel.values()[random.nextInt(5)]);
    ...
    private void createQuantitativeSingleAnswerQuestion(Integer number, AbstractQuantitativeSingleAnswerQuestion question) {
        question.setText("This is a sample Quantitative Single Answer gre " + number);
        question.setDifficulty(Difficulty.values()[random.nextInt(3)]);
        question.setDifficultyLevel(DifficultyLevel.values()[random.nextInt(5)]);
