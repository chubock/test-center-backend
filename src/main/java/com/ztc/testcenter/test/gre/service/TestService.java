...
import com.ztc.testcenter.test.gre.exceptions.*;
import com.ztc.testcenter.test.gre.repository.*;
import com.ztc.testcenter.user.domain.User;
import com.ztc.testcenter.question.gre.domain.Difficulty;
import com.ztc.testcenter.question.gre.domain.InnerQuestion;
import com.ztc.testcenter.question.gre.domain.Question;
import com.ztc.testcenter.question.gre.domain.QuestionsContainer;
import com.ztc.testcenter.question.gre.repository.QuestionRepository;
...
public class TestService {
    ...
    public List<Test> getUserTests(User user) {
        return testRepository.findByUserAndEndDateIsNotNull(user);
    }
    ...
    public Test getUserCurrentTest(User user) {
        Long testId = testRepository.getIdByUserAndEndDateIsNull(user);
        ...
        testSection.getTemplate().getItems().forEach(item -> {
            ...
            if (item.getQuestionTemplate() == null) {
                ...
                if (candidateQuestionsIds == null || candidateQuestionsIds.isEmpty()){
                    logger.warn("Couldn't find unique gre for user : " + testSection.getTest().getUser().getId() + ", difficulty: " + testSection.getTemplate().getDifficulty() + ", difficulty_level: " + item.getDifficulty() + ", free: " + testSection.getTest().getFree() + " (page_number: " + pageRequest.getPageNumber() + ", page_size: " + pageRequest.getPageSize());
                ...
                //Check if we can't find unique gre, then we have to find some questions anyway;
                if (candidateQuestionsIds == null || candidateQuestionsIds.isEmpty()){
                    logger.warn("Couldn't find unique gre for user : " + testSection.getTest().getUser().getId() + ", difficulty: " + testSection.getTemplate().getDifficulty() + ", question_template: " + item.getQuestionTemplate().getLabel() + ", free: " + testSection.getTest().getFree() + " (page_number: " + pageRequest.getPageNumber() + ", page_size: " + pageRequest.getPageSize());
